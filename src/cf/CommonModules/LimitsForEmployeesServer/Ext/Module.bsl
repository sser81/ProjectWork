...
Процедура ДО_ЗагрузитьДанныеОбмена(ОбъектыXDTO) Экспорт
	...
	For Each ОбъектXDTO In ОбъектыXDTO Do
		...
		If TableName = NStr("en = 'Catalog.ДО_ЭтапыЛимитовСотрудников';
							|ru = 'Справочник.ДО_ЭтапыЛимитовСотрудников'") Then

			СтатьяДвиженияДенежныхСредств = ДО_ПолучитьСсылкуПоОбъектуXDTO(ОбъектXDTO.CashFlowItem, "Catalog.ДО_СтатьиДвиженияДенежныхСредств", True);
			...
			Владелец = ДО_ПолучитьСсылкуПоОбъектуXDTO(ОбъектXDTO.Employee, "Catalog.ДО_Сотрудники", True);
...
Функция ДО_ПолучитьСсылкуПоОбъектуXDTO(ОбъектXDTO, ИмяТипа = "", ПолучатьСсылкуПоИдентификатору = Ложь) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИмяТипа) Тогда
		...
		Если ЕстьПолеТипа Тогда
			...
			Если СтрНайти(ВРег(ИмяТипаПолученныхДанных), ВРег("StageOfLimitsForEmployees")) > 0 Тогда
				ИмяТипа = NStr("en = 'Catalog.ДО_ЭтапыЛимитовСотрудников'; ru = 'Справочник.ДО_ЭтапыЛимитовСотрудников'");

			ИначеЕсли СтрНайти(ВРег(ИмяТипаПолученныхДанных), ВРег("CashFlowItems")) > 0 Тогда
				ИмяТипа = NStr("en = 'Catalog.ДО_СтатьиДвиженияДенежныхСредств'; ru = 'Справочник.ДО_СтатьиДвиженияДенежныхСредств'");

			ИначеЕсли СтрНайти(ВРег(ИмяТипаПолученныхДанных), ВРег("_DemoCompanies")) > 0 Тогда
				ИмяТипа = NStr("en = 'Catalog.ДО_Организации'; ru = 'Справочник.ДО_Организации'");
			...
			ИначеЕсли СтрНайти(ВРег(ИмяТипаПолученныхДанных), ВРег("_DemoIndividuals")) > 0
				...
				ИмяТипа = NStr("en = 'Catalog.ДО_Сотрудники'; ru = 'Справочник.ДО_Сотрудники'");
